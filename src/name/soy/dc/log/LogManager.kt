...
class LogManager(private val center: DataCenter) {
     ...
     */
    fun queryLog(start: Long,end: Long, predicate: Predicate<LogEntry>):List<LogData> = queryLog(start,end,predicate, Predicate { true })
    fun queryLog(start: Long,end: Long, entrypre: Predicate<LogEntry>,levelpre: Predicate<Level>):List<LogData> = queryLog(start,end,entrypre,levelpre, Predicate { true })
    fun queryLog(start: Long,end: Long, entrypre: Predicate<LogEntry>,levelpre: Predicate<Level>,msgpre: Predicate<String>):List<LogData>
            = queryLog({ time:Long-> time in start..end },entrypre,levelpre, msgpre)
